    ...
    <div className={`${darkMode ? 'dark' : ''} h-full`}>
      ...
      <div className="flex h-[100dvh] bg-gray-50 dark:bg-slate-900 font-sans text-slate-900 dark:text-gray-100 transition-colors duration-300 pt-[env(safe-area-inset-top,0px)]">
        ...
        <div className="flex-1 flex flex-col overflow-hidden w-full relative">
          {/* Top Header */}
          <header className="h-14 md:h-16 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between px-3 md:px-6 shadow-sm z-[900] relative transition-colors duration-300 flex-shrink-0">
            {isMobileScreen ? (
              // Mobile Header
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <HoubaraIcon className="w-7 h-7 text-brand-600 dark:text-brand-400" />
                  <span className="font-bold text-base tracking-tight text-gray-900 dark:text-white">RAF Tracking</span>
                </div>
                <div className="flex items-center gap-2">
                  <NotificationPanel onNavigate={handleNavigate} />
                  <div className="w-8 h-8 bg-brand-50 dark:bg-slate-700 rounded-full flex items-center justify-center text-brand-600 dark:text-brand-400 border border-brand-100 dark:border-slate-600">
                    <UserCircle size={18} />
                  </div>
                  <button
                    onClick={handleLogout}
                    title="Log out"
                    className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-400/10 rounded-lg transition-colors"
                  >
                    <LogOut size={18} />
                  </button>
                </div>
          ...
          <main className={`flex-1 overflow-hidden relative bg-gray-50/50 dark:bg-slate-900/50 flex flex-col ${isMobileScreen ? 'pb-16' : ''}`}>
            <div className="flex-1 overflow-y-auto p-3 md:p-6 w-full mx-auto flex flex-col h-full">
               {renderContent()}
            </div>
          </main>
